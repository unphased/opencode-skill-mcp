...
import type { McpServerConfig, SkillMcpClientInfo, SkillMcpManagerState, SkillMcpServerContext } from "./types"
...
export class SkillMcpManager {
  ...
  async getOrCreateClient(info: SkillMcpClientInfo, config: McpServerConfig): Promise<Client> {
    ...
    return await getOrCreateClient({ state: this.state, clientKey, info, config })
  }
  ...
  async listTools(info: SkillMcpClientInfo, context: SkillMcpServerContext): Promise<Tool[]> {
    const client = await this._getClientWithRetry(info, context.config)
  ...
  private async withOperationRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const client = await this._getClientWithRetry(info, config)
  ...
  private async _getClientWithRetry(info: SkillMcpClientInfo, config: McpServerConfig): Promise<Client> {
    const clientKey = this.getClientKey(info)
    return await getOrCreateClientWithRetry({ state: this.state, clientKey, info, config })
  }
